import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import { Link } from "react-router-dom";
import SectionHeader from "../../UI/SectionHeader";
import { imageKitUrl } from "../../../utils/imageKit";
import { usePrefersReducedMotion } from "../../../hooks/usePrefersReducedMotion";
import { projects } from "../../../data/projects";
import { hasCaseStudyContent } from "../../../types/website";
import "./Projects.css";

function projectHref(project) {
  if (project.link && project.link !== "#") return project.link;
  if (hasCaseStudyContent(project.caseStudy) && project.slug) {
    return `/projects/case-study/${project.slug}`;
  }
  return "/projects";
}

function isExternal(href) {
  return href.startsWith("http");
}

export default function Projects() {
  const containerRef = useRef(null);
  const galleryRef = useRef(null);
  const reducedMotion = usePrefersReducedMotion();

  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end end"],
  });

  const totalDistance = useTransform(scrollYProgress, [0, 1], [0, -1200]);

  const renderItem = (project, index) => {
    const href = projectHref(project);
    const external = isExternal(href);
    const content = (
      <>
        <img
          src={imageKitUrl(project.img, 800)}
          className="absolute inset-0 h-full w-full object-cover opacity-60"
          alt={project.title}
          width={800}
          height={600}
          loading="lazy"
          decoding="async"
        />
        <div className="item-content">
          <span className="item-number">0{index + 1}</span>
          <h2>{project.title}</h2>
        </div>
      </>
    );

    if (external) {
      return (
        <a
          key={project.slug || project.title}
          href={href}
          target="_blank"
          rel="noopener noreferrer"
          className="gallery-item relative block aspect-[4/3] overflow-hidden rounded-2xl"
        >
          {content}
        </a>
      );
    }

    return (
      <Link
        key={project.slug || project.title}
        to={href}
        className="gallery-item relative block aspect-[4/3] overflow-hidden rounded-2xl"
      >
        {content}
      </Link>
    );
  };

  if (reducedMotion) {
    return (
      <div id="example">
        <SectionHeader title={"Projects"} />
        <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-6 px-6 py-12 sm:grid-cols-2">
          {projects.map((project, index) => renderItem(project, index))}
        </div>
      </div>
    );
  }

  return (
    <div id="example">
      <SectionHeader title={"Projects"} />

      <div ref={containerRef} className="scroll-container">
        <div className="sticky-wrapper">
          <motion.div ref={galleryRef} style={{ x: totalDistance }} className="gallery">
            {projects.map((project, index) => {
              const href = projectHref(project);
              const external = isExternal(href);
              const inner = (
                <>
                  <img
                    src={imageKitUrl(project.img, 800)}
                    className="absolute inset-0 h-full w-full object-cover opacity-60"
                    alt={project.title}
                    width={800}
                    height={600}
                    loading="lazy"
                    decoding="async"
                  />
                  <div className="item-content">
                    <span className="item-number">0{index + 1}</span>
                    <h2>{project.title}</h2>
                  </div>
                </>
              );

              if (external) {
                return (
                  <a
                    key={project.slug || project.title}
                    href={href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="gallery-item"
                  >
                    {inner}
                  </a>
                );
              }

              return (
                <Link key={project.slug || project.title} to={href} className="gallery-item">
                  {inner}
                </Link>
              );
            })}
          </motion.div>
        </div>
      </div>
    </div>
  );
}
